...
module.exports = {
  list: async (req,res) => res.render('layouts/admin',{view:'../admin/habitaciones/list',data:await repo.findAll(), active: 'habitaciones'}),
  createForm: async (req,res) => res.render('layouts/admin',{view:'../admin/habitaciones/create',hoteles:await hotelRepo.findAll(),tipos:await tipoRepo.findAll(), active: 'habitaciones'}),
  create: async (req,res) => {await repo.create(req.body);res.redirect('/admin/habitaciones')},
  editForm: async (req,res) => res.render('layouts/admin',{view:'../admin/habitaciones/edit',data:await repo.findById(req.params.id),hoteles:await hotelRepo.findAll(),tipos:await tipoRepo.findAll(), active: 'habitaciones'}),
